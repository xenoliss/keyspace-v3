...
// //    - EC is not enforced for wallet upgrades, allowing users to upgrade their wallet to a working implementation
// even
...
// //      calls. This approach ensures that EC is only enforced when executing non-config transactions, which is
// aligned
...
// //      believe this tradeoff is worthwhile compared to the alternative risk of bricking all user wallets due to
// chain
// //      or protocol changes.
// //
...
// //
// // To further minimize risks associated with a revoked, non-preconfirmed signer upgrading to a custom implementation,
// we
...
// //      unsynced states.
// //
// // Implementing these restrictions in the future would further mitigate the security risks of allowing upgrades when
// the
...
//     /// @inheritdoc Keystore
//     function _applyConfigHook(ConfigLib.Config calldata config) internal override {
...
//         require(
//             SignatureCheckerLib.isValidSignatureNow({signer: sigUpdateSigner, hash: newConfigHash, signature:
// sigUpdate}),
